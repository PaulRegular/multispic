...
The degree of temporal correlation is controlled by $\phi$, where low to high correlation is represented by values between 0 and 1, and species-to-species correlations are described by $\rho_{s, s}$, where negative an positive correlation is represented by values between -1 and 1. This is a flexible structure that allows for the testing of alternate hypotheses that process errors are independent through time or across species (i.e, $\phi = 0$ or $\rho_{s, s} = 0$). The possibility that process errors are similarly correlated across all species may also be tested by estimating only one $\rho$ parameter. Finally, the magnitude of the process error deviations are controlled by the species-specific standard deviation parameters, $\sigma_s$.
