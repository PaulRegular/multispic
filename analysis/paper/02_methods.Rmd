...
where $B_y$ is biomass at the start of year $y$, $C_y$ is the catch through year $y$, and changes in biomass due to growth, recruitment, and natural mortality are captured via the production function $g(B)$ [@polacheck1993]. An early and influential form for the production function was based on the logistic equation,
...
Combining equations \@ref(eq:bdm) and \@ref(eq:logistic) results in the Schaefer production model and, by applying a state-space framework [sensu @meyer1999], the stochastic form of this population process may be informed by survey data with observation error. Specifically,

\begin{align}
B_y =& \left ( B_{y-1} + rB_{y-1} \left( 1 - \frac{B_{y-1}}{K} \right) - C_{y-1} \right ) e^{\delta_y} & (\#eq:schaefer) \\
I_{y,s} =& q_sB_ye^{\varepsilon_{y,s}} & (\#eq:obs)
\end{align}

where $\delta$ represents process error, $\varepsilon$ represents observation error, $I_{y,s}$ is an index of relative abundance in year $y$ from survey $s$, and $q_s$ is the time-invariant catchability coefficient for survey $s$.
