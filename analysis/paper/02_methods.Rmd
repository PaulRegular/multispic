...
The priors chosen for our case study were loosely based on priors used by Froese et al. [-@froese2017]. For simplicity, all priors were normally distributed and the upper and lower inflection points ($\mu \pm \sigma$; ~68% of the total area under the curve) of each normal prior were defined using values in log or logit space. For $\log(r)$, $\log(0.01)$ and $\log(0.1)$ were chosen as the upper and lower inflection points. 
