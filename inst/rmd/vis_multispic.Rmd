...
named_par <- lapply(name_par, 
                    function(nm) {
                        if (grepl("log_", nm)) return(exp(fit$par[[nm]]))
                        if (nm == "logit_rho") return(inv_logit(fit$par[[nm]], shift = TRUE))
                        if (nm == "logit_phi") return(inv_logit(fit$par[[nm]], shift = FALSE))
                    })
...
names(named_par$phi) <- ""
names(named_par$rho) <- sp_nm_mat[lower.tri(sp_nm_mat)]

## TODO: convert to table with se and upper lower vals. Add betas

knitr::kable(named_par, "pipe")
