...
// Atomic double specifies the forward mode derivative, which is just pnorm(x, log.p = TRUE) = log(pnorm)
// Reverse is the explicit derivative of log(pnorm) = dnorm / pnorm = exp(log(dnorm) - log(pnorm)),
// more stable derivatives at extreme values.
TMB_ATOMIC_VECTOR_FUNCTION(
			   ...
			   px[0] = exp(ldnorm(tx[0]) - pn) * py[0]; //derivative of log(pnorm) in a more computationally stable way
			   )
...
template<class Type>
Type pnorm4(Type q, Type mean = 0.0, Type sd = 1.0, int give_log = 1){
  if (give_log == 0) {
...
// Function to calculate log(pnorm(x + upper, mu, sd) - pnorm(x - lower, mu, sd))
// with stable derivatives
template<class Type>
...
// Leverage the censored_b functions to create an approximation of dmuniform
// This is called a mollified uniform density using a Gaussian distribution
// https://stats.stackexchange.com/a/332558
template<class Type>
Type dmuniform(Type x, Type lower, Type upper, Type sd) {
